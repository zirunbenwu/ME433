...
# --- plot ---
PLOT_LEN   = 320          # samples held in the rolling buffer
...
def update(dt):
    ...
    # push samples into the rolling plot buffers (drop oldest, append newest)
...
def draw_plot(x, y, w, h, data, ymax, color, label, baseline_mid=True):
    """Draw a rolling time-series plot in a box at (x,y) size (w,h).
    baseline_mid=True puts zero in the vertical center (for signed signals)."""
    # frame
...
def draw():
    ...
    # slosh: signed, autoscale a bit above current activity
