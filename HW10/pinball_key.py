...
# ----- Physics constants ---------------------------------------------------
GRAVITY = 500.0           # px / s^2 (lower = lighter ball, slower fall)
BALL_R = 10
...
LAUNCH_LANE_TOP = 150            # y where the launch channel opens into the rail
...
def resolve_flipper(pivot_x, pivot_y, angle, prev_angle):
    ...
    # When the closest point is on the segment interior, use the flipper's
    # "outward" normal -- the perpendicular pointing toward the playable
    # upper side (smaller y in screen coords). This prevents the ball from
    # being pushed *through* the flipper when it ends up slightly below the
    # line (fast falls, edge cases, or marginal tunneling).
    # When the closest point is an endpoint (pivot or tip), fall back to the
    # vector from endpoint to ball so the ball rolls naturally around corners.
    if 0.0 < t < 1.0:
    ...
    # Push the ball to the outward side of the flipper.
    ball_x = cx + nx * radius_sum
    ball_y = cy + ny * radius_sum
    # Reflect velocity if moving into the flipper.
    vn = ball_vx * nx + ball_vy * ny
    if vn < 0:
        ball_vx -= (1 + WALL_BOUNCE) * vn * nx
        ball_vy -= (1 + WALL_BOUNCE) * vn * ny
...
def update(dt):
    ...
    launch = (btn_left and btn_right) or keyboard.space
...
def draw():
    ...
    if game_over:
        ...
        screen.draw.text("Press both buttons/space to restart",
                         center=(WIDTH / 2, HEIGHT / 2 + 80),
                         color=(200, 200, 200), fontsize=22)
    elif not ball_active:
        screen.draw.text("Press both buttons/space key to launch",
                         center=(WIDTH / 2, HEIGHT / 2+100),
                         color=(220, 220, 220), fontsize=24)
        screen.draw.text("Use left and right arrow to control the flipper",
                         center=(WIDTH / 2, HEIGHT / 2 + 164),
                         color=(220, 220, 220), fontsize=20)
